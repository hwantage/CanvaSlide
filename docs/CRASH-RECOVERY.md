...
| Native   | `<app data>/recovery/<session>.json`. Write `<session>.json.tmp`, synchronize the file, then replace the destination through the shared [atomic replace](../src-tauri/src/atomic_file.rs). Unix also synchronizes the parent directory where the volume supports it; Windows uses `MoveFileExW` with replacement and write-through flags, falling back to the standard rename when that is refused. Failures propagate before the store retires an adopted copy. |
...
succeed. Browsers use an exclusive Web Lock; native instances use OS file locks
(`std::fs::File::lock`).
