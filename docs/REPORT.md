...
- **파일 저장 컨테이너(v3)**: `.canvaslide`는 `document.json`과 `resources/<내용 ID>`를 담은 ZIP이다.
  자산의 `source`는 원문 문자열, Base64 이미지 바이트 또는 SVG 템플릿·비트맵 삽입 위치를 참조한다.
  SVG 안의 같은 비트맵과 독립 이미지를 하나의 리소스로 공유하며, 문서·SVG 마크업만 Deflate로 압축한다.
  열 때 기존 data URL을 복원하므로 요소·클립보드·HTML 플레이어의 v2 모델은 유지한다. v1/v2 JSON도 읽는다.
  ZIP CRC·중복 항목·리소스 참조·압축 해제 및 복원 크기를 검사한다. 저장/열기와 네이티브 전송 변환은 워커에서 처리한다.
  연속 저장은 변경되지 않은 자산의 분석·압축 결과를 재사용하며, 워커 캐시는 현재 문서의 리소스만 보관하고
  30초 유휴 후 해제한다. 새 v3 파일을 읽으려면 이 형식을 지원하는 앱이 필요하다.
...
4. **이미지 저장 형식**: 저장 파일은 v3 ZIP으로 SVG 내부 비트맵까지 공용 리소스로 묶고 마크업을 압축한다.
   v1/v2 JSON은 계속 읽으며 실행 중에는 기존 v2 모델로 복원한다. 런타임 메모리·HTML 내보내기 용량은 줄이지 않는다.
   다시 저장한 v3 파일은 이전 앱에서 열 수 없다. 압축·복원은 워커에서 실행하며 연속 저장은 순서대로 처리한다.
