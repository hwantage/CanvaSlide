import type { SiteStrings } from '../site-strings'
...
export const ko: SiteStrings = {
  ...
  'site.docs.sharing.cloudBody':
    '클라우드 공유는 실험 기능입니다. 호스팅 서비스의 제한과 가용성이 {cloudflare} 요금제에 좌우됩니다. 공유에서 사본 편집 또는 슬라이드 쇼만 보기를 선택한 뒤 링크 복사를 누르세요. 발표 전용 링크에는 하나 이상의 프레임이 필요하며 편집 도구 없이 열립니다. 링크를 아는 사람은 24시간 동안 스냅샷에 접근할 수 있습니다. 이후 편집은 반영되지 않으며 계정, 수동 링크 폐기와 실시간 공동 편집은 제공하지 않습니다. 내장 이미지를 포함한 스냅샷 한도는 {limit}입니다. 공유를 사용할 수 없거나 크기를 초과하면 로컬 저장 또는 {html} 내보내기를 사용하세요. 발표 전용 화면도 콘텐츠 복사를 막지는 못합니다.',
  ...
  'site.docs.faq.networkA':
    '분석, 원격 측정, 오류 보고 기능은 없습니다. 데스크톱 앱은 기본적으로 실행할 때마다 3초 뒤 {github}에서 최신 릴리스를 확인합니다. 이 요청에는 문서 내용, 앱 버전, 기기 식별자가 없으며 업데이트는 직접 설치를 선택해야 설치됩니다. 이 확인을 끄려면 정보 대화상자에서 시작할 때 업데이트 확인을 해제하세요. 같은 대화상자의 업데이트 확인으로 필요할 때 직접 확인할 수 있으며, 오프라인이거나 차단되어도 앱은 그대로 동작합니다. 링크 복사는 문서 전체를 공유 서비스에 올리며 24시간 동안 보관됩니다. 동영상 링크를 추가하거나 재생하면 {youtube}, {vimeo} 또는 동영상 서버에 연결합니다. 자동 재생으로 설정된 동영상(기본값)은 발표 중 해당 프레임에 도착하면 클릭하지 않아도 불러오기 시작합니다. 한 프레임에서 이렇게 불러오는 {youtube} 동영상은 하나뿐이며, 재생하려면 브라우저나 제공 서비스가 클릭을 요구할 수 있습니다. {youtube} 동영상이 있는 문서는 썸네일을 {youtube}에서 불러옵니다. 웹 편집기는 호스팅된 페이지이지만 문서는 공유하기 전까지 브라우저에 남습니다.',
