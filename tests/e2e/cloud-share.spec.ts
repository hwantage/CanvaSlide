...
test('creates, copies, and opens a snapshot with its content fitted @webkit', async ({
  ...
  const dialog = page.getByRole('dialog', { name: 'Cloud share', exact: true })
  ...
  await expect(dialog.getByRole('radio', { name: /Edit a copy/ })).toBeChecked()
  await expect(dialog.getByRole('radio', { name: /View slide show only/ })).toBeDisabled()
...
test('publishes a slideshow-only link with navigation and no path back to editing @webkit', async ({
  ...
  await page.getByRole('button', { name: 'Share', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Cloud share' })
  await dialog.getByRole('radio', { name: /View slide show only/ }).check()
  await dialog.getByRole('button', { name: 'Create link' }).click()
  const field = dialog.getByRole('textbox', { name: 'Shareable URL' })
  await expect(field).toHaveValue(/\?share=[\w-]{21}$/)
  const url = await field.inputValue()
  await expect(dialog.getByRole('radio', { name: /View slide show only/ })).toBeChecked()
  await expect(dialog.getByRole('radio', { name: /Edit a copy/ })).toBeDisabled()
...
test('quota failure offers a working local document download and a retry @webkit', async ({
  ...
  await dialog.getByRole('button', { name: 'Save .canvaslide file' }).click()
