...
### 3.5 JSON 문서와 HTML 익스포트

- **파일 규격**: `.canvaslide`는 `version: 1`인 단일 UTF-8 JSON이다. `elements`는 ID를 키로
  하는 객체이며, `order`·`settings`·`assets`·`resources`를 함께 저장한다. 이전 JSON/ZIP 규격의
  읽기·변환은 제공하지 않는다. 파일 규격은 `documentFileSchema`가 정의한다.
- **자산 중복 제거**: 이미지 요소는 `assetId`로 자산 메타데이터를 참조하고, 자산은 `resourceId`로
  공유 리소스를 참조한다. 이미지 data URL은 한 번 저장하며, SVG는 원문 문자열과 데이터 리소스
  참조를 조합한 `parts`로 저장한다. 열 때 렌더러용 data URL을 복원한다.
- **저장/열기**: 파일 크기·스키마·리소스 참조·복원 크기를 검사한다. 변환은 워커에서 처리하며
  연속 저장은 변경되지 않은 자산의 분석 결과를 재사용한다. 캐시는 현재 문서의 리소스만 보관하고
  30초 유휴 후 해제한다. 상세 제작 규칙은 `examples/README.md`에 있다.
- **단일 HTML 플레이어**: `src/player/`의 React 없는 바닐라 뷰어가 `src/shared/canvas`의
  카메라·보간·피팅·순서 로직을 그대로 import하므로 앱과 동일하게 움직인다. Vite lib 빌드
  (`config/vite.player.config.ts`)로 번들한 뒤 앱이 `?raw`로 인라인한다. 연결된 동영상이 없는 문서는 외부 요청 없이 오프라인에서 열린다.
...
### 3.11.1 Figma 파일 가져오기 (2026-09-18)

- **파일 가져오기**(⌘I / Ctrl+I)와 캔버스 드롭에서 `.fig`를 받는다. 페이지를 선택하고,
  텍스트·기본 도형을 편집 가능한 요소로 변환하거나 최상위 레이어별 이미지로 유지한다.
  페이지 내부 좌표·순서를 보존하고 페이지를 나란히 배치하며, 페이지와 주요 디자인 프레임을 발표 프레임으로 만든다.
- ZIP / raw `fig-kiwi`, Deflate / Zstandard와 파일 내장 Kiwi 스키마를 로컬 워커에서 해독한다.
  스키마를 해석하여 실행하므로 `eval` 없이 기존 CSP에서 동작한다. 취소·오류 시 문서를 변경하지 않고,
  성공 시 자산과 요소를 한 번에 추가해 undo 1단계를 만든다. 대량 삽입의 반복 문서 복사도 제거했다.
- 이미지 자르기, 계층 행렬, 클리핑, 알파 마스크, quadratic/cubic 벡터·텍스트 글리프를 SVG로 처리한다.
  동일 비트맵 및 위치만 다른 SVG의 중복 저장을 줄인다. 효과·인스턴스 참조 확장·고급 채우기 등 제한은
  결과 창에 표시한다. 상세 범위·재현 방법은 [FIGMA-IMPORT.md](./FIGMA-IMPORT.md).
- 제공된 `figma1.fig`(약 36MB, 버전 106): 내부 페이지를 제외한 12페이지, 원본 노드 4,984개를
  CanvaSlide 요소 4,257개(텍스트 1,419, 도형 341, 이미지 2,428, 프레임 69)로 변환했다.
  생성 파일은 약 63MiB이며, 사용자 원본·변환 파일은 저장소에 포함하지 않는다.
- 편집 모드에서 프레임·부분 서식 텍스트가 이미지로 합쳐지던 문제를 수정했다. 텍스트가 있는 컨테이너는
  배경과 자식을 쌓임 순서대로 분리하여 표시 대상 원본 텍스트 1,419개 모두를 편집할 수 있다.
  글꼴·기울임·줄 간격과 사각 프레임 자르기를 유지하며, 부분 서식·회전은 근사한다.
  더블클릭 편집·실행 취소·저장·다시 열기를 실제 사용자 샘플과 별도 합성 파일로 검증한다.
- 코드 검토 후 빈 SVG 요소 35개를 제거하고, 중복 범위 계산·자식 목록 생성·전체 결과 보관을 정리했다.
  대량 삽입은 순수 문서 변환으로 옮겨 복제·붙여넣기에서도 재사용한다. import 세션별로 워커를 관리하여
  취소한 파일의 늦은 실패가 다음 import에 영향을 주지 않으며, 실패한 워커도 즉시 해제한다.
...
- **전환 미리보기 모드**: 절 제목 옆 재생 버튼이 `previewTransition(frameId)`를 호출한다(순서 인덱스가 아니라
  **프레임 id**로 든다 — 미리보기가 멈춰 있는 동안에도 목록에서 덱을 재정렬할 수 있기 때문이다). 직전 프레임의
  ...
  여러 프레임을 선택하면 목록 순서의 첫 프레임부터 미리보기를 시작한다. 오른쪽 위 알약의 이전·다음 버튼이나
  방향키·Space·PageUp/PageDown으로 선택한 프레임 사이만 이동하며, 처음과 끝에서 멈춘다. 선택한 ID는 재생 중
  유지하고, 순서를 바꾸면 현재 목록 순서를 따른다. 다시 재생·닫기가 있고 Esc로도 닫힌다. 대기는 애니메이터의 `holdMs` 옵션으로 처리해
  ...
  `active` 그대로다(기울어진 화면에서 히트테스트가 어긋나면 안 된다). 키보드도 미리보기 종료·이동 키만 가져가며, 속성 입력에 포커스가 있으면 방향키는 그 입력을 조절한다.
  ...
  날아갔다. 미리보기 중에도 Delete는 편집기로 그대로 내려가 보던 프레임이 지워질 수도
  있다 — 그때는 자리를 물려받은 프레임에 다시 맞추는 대신 `cancelPreview()`로 카메라를 놓는다.
- **프레임 다중 선택과 일괄 편집**: 목록에서 ⌘/Ctrl+클릭은 개별 토글, Shift+클릭은 기준 행부터 범위를
  선택하고, 일반 클릭은 단일 선택과 화면 맞춤을 실행한다. 목록이나 캔버스에서 프레임만 여러 개 선택하면
  카메라 속성을 함께 편집할 수 있다. 서로 다른 값은 `혼합`으로 표시하며, 한 항목을 바꿔도 각 프레임의 다른
  설정은 유지한다. 프레임만 선택하면 정렬·분배 도구는 표시하지 않는다. 단계 선택·초기화·고급 슬라이더
  한 번 끌기는 각각 선택 전체에 대한 실행 취소 한 단계다.
...
- 도구 모음의 **동영상 링크 삽입**, 선택한 동영상의 **링크 편집**, 캔버스의 ⌘V / Ctrl+V로
  YouTube·Vimeo·직접 영상 URL을 넣는다. 붙여넣기는 알려진 공급자 링크와 영상 확장자를 인식하고,
  확장자가 없는 스트리밍 URL은 삽입 대화상자에서 입력한다. 객체·이미지·일반 텍스트·입력칸 붙여넣기는 유지한다.
- 붙여넣기·링크 삽입 시 직접 영상의 메타데이터나 공급자 oEmbed 치수로 가로·세로 비율을 맞추고,
  영상 아래 오른쪽 아이콘 제어 영역은 별도로 확보한다. 정보가 없으면 가로형, Shorts 링크는 세로형으로 시작한다.
  조회는 제한 시간 안에 끝내며 중간 편집·실행 취소·문서 교체 후에는 자동 크기 변경을 취소한다. 삽입은 한 번에 실행 취소된다.
- `video` 요소에는 URL, 위치·크기와 자동재생 설정만 저장한다. 영상 업로드·미디어 다운로드·바이너리 자산 저장은 제공하지 않는다.
  직접 URL의 서명 쿼리, Vimeo 비공개 링크의 해시와 공급자 링크의 시작 시간을 보존한다.
- YouTube는 재생 전에도 원격 썸네일을 표시하고, 해상도별 이미지가 없으면 기본 안내로 돌아간다.
  썸네일 파일은 문서 자산으로 저장하지 않으며 HTML 플레이어에서도 같은 방식으로 표시한다.
- 편집기는 **재생**을 누를 때만 재생기와 공급자 SDK를 불러온다. 슬라이드 쇼는 현재 프레임에 완전히 포함된 영상만
  도착 후 **음소거로 자동 재생**하며 소리 켜기·원래 플레이어 제어를 제공한다. 동영상 속성의
  **슬라이드 쇼에서 자동 재생**을 해제하면 수동 재생으로 시작하고 저장·HTML 내보내기에도 유지한다. 기본값은 켜짐이다. 복사한 프레임의 내용물 판정도
  프레임 이동과 같은 규칙을 따른다. YouTube 정책에 따라 같은 화면의 YouTube 자동재생은 첫 영상 하나로 제한한다.
- 오른쪽의 **일시 중지 / 재개**는 재생 위치와 같은 플레이어를 유지한다. 공급자 SDK가 준비되기 전의
  일시 중지 요청도 준비 후 반영한다. **확대 보기**는 하단 앱 컨트롤·테두리·네비게이션용 여백 없이
  영상 프레임을 캔버스 전체에 맞춘다. 원본 영상은 비율을 유지하고, 화면 비율과 다르면 검은 여백을 사용한다.
  이전·다음 네비게이션은 영상 위에 겹쳐 표시한다. 직접 영상의 기본 컨트롤은 확대 중 숨기고 영상 클릭이나
  영상에 포커스한 상태의 Space로 일시 중지·재개한다. 우측 상단 **이전 보기로 돌아가기** 또는 Esc로 원래 보기에 복귀한다. 확대·복귀·창 크기 변경은 영상 재생이나 문서 크기를 초기화하지 않으며
  편집기·슬라이드 쇼·HTML 플레이어에서 동일하게 동작한다.
- 프레임 변경·전체 보기·발표 종료·문서 교체·요소 삭제 때 영상과 iframe을 제거하고 늦은 로딩 콜백을 무효화한다.
  다시 방문하면 링크의 시작 시간 또는 처음부터 재생한다. 자동재생 차단·로드 실패에는 재생 안내·재시도·원본 열기를 제공한다.
- YouTube의 HTTP Referer 요구를 충족하기 위해 배포 앱은 필요할 때만 루프백 HTTP 페이지를 띄운다.
  이 페이지는 고정된 플레이어 파일 두 개만 제공하며 파일 접근·미디어 프록시·Tauri IPC 권한은 없다.
  공급자 SDK·iframe 출처는 CSP에 한정하고, 페이지 간 메시지의 출처와 창을 확인한다.
- HTML 내보내기도 같은 링크와 재생 수명주기를 유지한다. 연결된 영상에는 인터넷이 필요하며,
  **YouTube는 HTML을 HTTP(S) 주소로 열어야 한다**. 디스크에서 직접 열면 그 조건과 원본 열기를 안내한다.
  직접 영상의 코덱·인증·혼합 콘텐츠와 공급자의 삽입 제한은 브라우저 및 원본 설정을 따른다.
...
| 22  | 슬라이드쇼 비행 전후로 텍스트·이모지가 움찔거림                                                                 | 출발 시 레이아웃 배율을 1로 내렸다 도착 후 올려 두 번 재레이아웃. WebKit 시스템 폰트 자간은 크기별로 최대 14% 다르고 이모지 폭은 반올림됨    | 가벼운(비합성) 문서의 슬라이드쇼는 배율 1로 레이아웃하고 transform만 적용(`worldLayoutZoom`). 편집기·미리보기와 합성 문서는 정지 후 `zoom`을 커밋하고, 도착 배율이 출발 레이아웃 이하인 비행은 도착 배율로 비행(`flightLayoutZoom`). WebKit에서만 `font-optical-sizing: none`. 이미지 상세는 공통 원점 아래 픽셀 크기의 CPU 캔버스 한 장을 transform으로 배치하고 미리보기와 동시에 교체(`imageSurfaceStyle`, `mergeDetailTiles`) |
...
3. **성능**: DOM 렌더링이라 요소 수천 개부터 느려질 수 있다. 벡터 256개 이상인 문서만
   `will-change: transform`으로 합성한다. 편집기에서는 합성 힌트가 없어도 WebView가 변환된 콘텐츠를
   레이아웃 배율로 래스터할 수 있으므로, 요소 수와 관계없이 카메라가 멈춘 뒤(`ZOOM_SETTLE_MS`, 120ms)
   CSS `zoom`을 실제 확대 배율에 맞춘다. 제스처·전환 중에는 `transform: scale`로 움직이고 정지 후 한 번만
   레이아웃한다(`world-layer.tsx`, `use-settled-zoom.ts`). 가벼운 문서의 슬라이드쇼는
   글자·이모지의 재레이아웃을 방지하도록 배율 1을 유지한다(`worldLayoutZoom`, HTML 플레이어도 동일).
   CAMERA 미리보기는 요소 수와 관계없이 출발 대기에서 정지 배율을 유지하고, 도착 즉시 실제 배율로 복원한다.
   합성 문서는 슬라이드쇼에서도 정지 후 실제 배율로 레이아웃하며, 편집기로 돌아오면 모든 문서가 정착 정책을 복원한다.
   100% 미만에서는 `zoom`을 쓰지 않고
   ...
   차이였다. 슬라이드쇼의 가벼운 문서와 WebKit은 x·y·폭 차이 0.1px 미만으로 검사한다. WKWebView는 SVG `<img>`와 가속 캔버스를
   ...
   픽셀/월드 비율로 균등하게 나누고, 비트맵 샘플링 범위를 한 소스 픽셀 넓혀 보간 경계의 알파 손실을 방지한다
   (`imageDetailSamplingRect`; 출력 타일은 겹치지 않는다). 겹친 반투명 사진의 상세는 카메라별로 모두 준비된 뒤
   한 프레임에 미리보기와 교체한다(`image-detail-reveal.ts`). 두 표면이 동시에 보이는 페이드는 반투명 영역의
   알파를 이중 합성하므로 쓰지 않는다. 요소 524개 문서 기준 프레젠테이션 전환이
   ...
   새 상세 타일이 필요하다. 편집 동작 중에는 보관한 상세 타일을 숨기며, 해당 요소·에셋이 변경되거나 화면 밖으로
   이동하면 이전 리스를 해제한다. 화면 안에 있어도 축소 후
...
4. **이미지 저장 형식**: 저장 파일은 단일 UTF-8 JSON이며 SVG 내부 비트맵까지 공용 `resources`에 모은다.
   이전 JSON/ZIP 규격의 읽기·변환은 제공하지 않는다. 이미지 Base64와 SVG 텍스트를 압축하지 않아 ZIP보다 파일이 커질 수 있다.
   실행 중에는 자체 포함 자산으로 복원하므로 런타임 메모리·HTML 내보내기 용량은 별개다.
   직렬화·복원은 워커에서 실행하며 연속 저장은 순서대로 처리한다.
