...
describe('document-store', () => {
  ...
  it('tracks saved state and renames the document after the saved file', () => {
    const store = useDocumentStore.getState()
    store.insertElement(text('a'))
    const snapshot = store.takeSaveSnapshot()
    store.completeSave(snapshot, '/tmp/a.canvas.json', 'a')
    expect(useDocumentStore.getState().dirty).toBe(false)
    expect(useDocumentStore.getState().filePath).toBe('/tmp/a.canvas.json')
    expect(useDocumentStore.getState().document.name).toBe('a')
    expect(useDocumentStore.getState().past).toHaveLength(1)
  ...
  it('ignores a save that finishes after switching to another document', () => {
    ...
    store.completeSave(snapshot, '/tmp/old.canvas.json', 'old')
    const state = useDocumentStore.getState()
    expect(state.filePath).toBeNull()
    expect(state.dirty).toBe(true)
    expect(state.document.name).not.toBe('old')
  })
})
