...
| G9  | 파일: 새로 만들기 / 열기 / 저장 / 다른 이름으로 저장 (`.canvas.json`)                                     |
| G10 | Undo/Redo                                                                                                 |
...
- 보간: **van Wijk & Nuij (2003) "Smooth and efficient zooming and panning"** 을 구현.
  ...
  가까운 거리는 직선에 가까운 궤적이 된다. 사용자는 **전환 시간(ms)** 만 지정한다.
- 전환 시간이 고정이므로 먼 거리는 빠르게, 가까운 거리는 천천히 움직이는 것처럼 느껴진다(G8).
- `requestAnimationFrame` 루프에서 `t∈[0,1]`를 이징(easeInOutCubic) 후 보간기에 입력.
