...
vi.mock('./recovery-database', async (importOriginal) => {
  ...
  return {
    ...
    readStoredSnapshot: read,
    readStoredMetadata: read,
...
const envelope = JSON.stringify({ ...info, contents: '{"version":1}' })
...
beforeEach(() => {
  ...
  vi.mocked(inspectRecoveryFile).mockImplementation(async (payload) => {
    try {
      return inspectRecoverySnapshot(JSON.parse(payload as string))
    } catch {
      return { kind: 'quarantined', version: null }
    }
  })
})
describe('recovery backend boundaries', () => {
  ...
  it('preserves missing records and worker failures as retryable restore failures', async () => {
    ...
    vi.mocked(readStoredSnapshot).mockResolvedValue(envelope)
...
describe('launch scan', () => {
  const store = (records: Record<string, RecoveryDatabase.StoredSnapshot>) => {
    vi.mocked(listStoredSessions).mockResolvedValue(Object.keys(records))
    vi.mocked(readStoredSnapshot).mockImplementation(async (id) => records[id] ?? null)
  }
  it('uses new-format metadata without loading the payload into the worker', async () => {
    store({ lost: { info, payload: new Blob([bytes]) } })
    const result = await readRecoverySnapshots()
    expect(result.sessions[0]?.snapshot).toEqual(info)
    expect(inspectRecoveryFile).not.toHaveBeenCalled()
  })
  it('inspects legacy payloads on the worker and holds their claim', async () => {
    store({ lost: envelope })
    expect((await readRecoverySnapshots()).sessions[0]?.snapshot).toEqual(info)
    expect(inspectRecoveryFile).toHaveBeenCalledWith(envelope)
    expect(releaseRecoverySession).not.toHaveBeenCalled()
  })
  it('skips its own and adopted records even on a repeated scan', async () => {
    store({ own: envelope, adopted: envelope })
  ...
  it.each(['not json', JSON.stringify({ ...info, version: 2 })])(
    'never reads live incompatible data (%s)',
    async (payload) => {
      store({ live: payload })
  ...
  it.each([
    ['not json', null],
    [JSON.stringify({ ...info, version: 2 }), 2]
  ] as const)('quarantines abandoned incompatible data (%s)', async (payload, version) => {
    store({ lost: payload })
  ...
  it('reports an incomplete scan and releases the unreadable record for retry', async () => {
    store({ lost: envelope })
    vi.mocked(readStoredSnapshot).mockRejectedValue(new Error('store busy'))
