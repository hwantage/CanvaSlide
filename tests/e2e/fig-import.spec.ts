...
test('local sample: imports all pages, edits nested text and reopens the saved document @webkit', async ({
  page
}, testInfo) => {
  test.skip(!process.env.CANVASLIDE_FIG_SAMPLE)
  test.setTimeout(180_000)
  const chooser = page.waitForEvent('filechooser')
  await page.getByTestId('import-files').click()
  await (await chooser).setFiles(process.env.CANVASLIDE_FIG_SAMPLE!)
  const dialog = page.getByRole('dialog', { name: 'Import Figma file' })
  await expect(dialog.getByRole('checkbox')).toHaveCount(12, { timeout: 30_000 })
  await dialog.getByRole('button', { name: 'Toggle all pages' }).click()
  await dialog.getByRole('button', { name: 'Import selected pages' }).click()
  await expect(dialog).toContainText('Imported 12 pages', { timeout: 90_000 })
  await dialog.getByRole('button', { name: 'Close', exact: true }).click()
  await expect(page.getByTestId('frame-row').first()).toBeVisible()
  await expect(page.locator('[data-element-type="text"]')).toHaveCount(1419)
  await expect
    .poll(() =>
      page
        .locator('img[data-element-type="image"]')
    ...
    .toBeGreaterThan(0)
  await page.getByTestId('frame-row').nth(4).click()
  const target = page
    .locator('[data-element-type="text"]')
    .filter({ hasText: /^검사 규칙$/ })
    .first()
  await expect.poll(async () => (await target.boundingBox())?.height ?? 0).toBeGreaterThan(8)
  const targetId = await target.getAttribute('data-element-id')
  await editText(page, target, '규칙 수정')
  await page.screenshot({ path: testInfo.outputPath('figma1-text-edited.png') })
  const primary = await primaryModifier(page)
  const downloaded = page.waitForEvent('download')
  await page.keyboard.press(`${primary}+s`)
  const download = await downloaded
  ...
  const openChooser = page.waitForEvent('filechooser')
  await page.keyboard.press(`${primary}+o`)
  await (await openChooser).setFiles(reopened)
  await expect(page.getByTestId('frame-row')).toHaveCount(69, { timeout: 30_000 })
  await expect(page.locator('[data-element-type="text"]')).toHaveCount(1419)
  const reopenedText = page.locator(`[data-element-id="${targetId}"]`)
  await expect(reopenedText).toHaveText('규칙 수정')
  await page.getByTestId('frame-row').nth(4).getByRole('button').first().click()
  await expect.poll(async () => (await reopenedText.boundingBox())?.height ?? 0).toBeGreaterThan(8)
  await editText(page, reopenedText, '검사 규칙')
})
