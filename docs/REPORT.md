...
3. **성능**: DOM 렌더링이라 요소 수천 개부터 느려질 수 있다. 벡터 256개 미만의 가벼운 문서는 합성하지 않고
   항상 배율 1로 레이아웃한 채 `transform`만으로 움직인다(`worldLayoutZoom`, 플레이어 동일). 비합성 레이어는
   CTM으로 실제 배율에 그리므로 CSS `zoom` 재레이아웃과 똑같이 선명하고, 배율이 바뀌어도 글자·이모지가 한 번도
   재레이아웃되지 않는다. 벡터가 많은 문서만 `will-change: transform`으로 합성하는데, 합성 레이어는 레이아웃
   배율로 래스터되므로 제스처·전환 중에는 `transform: scale`로 스케일하고 카메라가 멈춘 뒤(`ZOOM_SETTLE_MS`)
   `zoom`을 한 번만 적용한다(`world-layer.tsx`, `use-settled-zoom.ts`). 100% 미만에서는 `zoom`을 쓰지 않고
