...
## Enforced invariants and regression evidence

The tests below exercise the named mechanism. Store/hook tests use controlled IO promises to create
race schedules; they establish state/ordering rules, not real disk durability. Browser integration
tests exercise actual IndexedDB, Web Locks and workers. Native tests use real files and OS locks.

| Invariant                                                                                                           | Direct regression evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| An offer/adopted record remains exclusively owned, and live incompatible data is skipped before inspection.         | `exclusive claims protect offered and adopted work...` and `live malformed and future envelopes...` in [recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts); `native_claims_are_exclusive_across_processes` and `two_native_instances_cannot_claim_read_or_delete_live_work` in [recovery_store_tests.rs](../src-tauri/src/recovery_store_tests.rs).                                                                                                                                                                                                               |
| Transient Restore errors do not authorize deletion.                                                                 | `transient restore read failure preserves the actual record and Retry succeeds` in [recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts) aborts a real IndexedDB transaction, checks retention, then restores.                                                                                                                                                                                                                                                                                                                                                      |
| A stale write cannot retire another document's adopted copy; a failed write retains the source.                     | Deferred/failing write and queued-stale-write cases in [recovery-store.test.ts](../src/renderer/src/store/recovery-store.test.ts) exercise the real store's session/adoption guards. Backend ordering is separately tested by real aborted IndexedDB replacement in [recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts) and the real-file tests `failed_replacement_keeps_the_old_copy` / `failed_file_sync_cannot_publish_or_acknowledge_a_replacement` (injected synchronization error) in [recovery_store_tests.rs](../src-tauri/src/recovery_store_tests.rs). |
| Restore stays dirty until a real save, including edit/undo.                                                         | Atomic restore tests in [document-store.test.ts](../src/renderer/src/store/document-store.test.ts), edit/undo in [recovery-store.test.ts](../src/renderer/src/store/recovery-store.test.ts), and the browser restore/Save flow in [recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts).                                                                                                                                                                                                                                                                            |
| Unanswered and unknown-version work is retained across prompt batches and reload; quota refuses rather than evicts. | Seven-session, future-version, record-quota and unload tests in [recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts); native quota/quarantine/temp tests in [recovery_store_tests.rs](../src-tauri/src/recovery_store_tests.rs); batch-versus-retention ordering in [recovery-snapshot.test.ts](../src/shared/canvas/recovery-snapshot.test.ts).                                                                                                                                                                                                                   |
...
| A failed temp does not hide the completed copy, and separately retained data survives cleanup of that copy.         | `corrupt_temp_does_not_hide_or_replace_the_completed_copy`, `oversized_temp_does_not_hide_the_completed_copy` and Unix `unreadable_temp_does_not_hide_the_completed_copy` in [recovery_store_tests.rs](../src-tauri/src/recovery_store_tests.rs) use actual files. [recovery-native-transport.test.ts](../src/renderer/src/platform/recovery-native-transport.test.ts) checks that the warning and offer both cross the IPC boundary.                                                                                                                                               |

The browser case `renderer text remeasurement does not cancel a delayed Open` in
[recovery-retention.spec.ts](../tests/e2e/recovery-retention.spec.ts) changes rendered line height,
waits for real ResizeObserver delivery, then finishes the delayed file read. Measurement-only
confirmation/read/cleanup and already-dirty live input are also covered in
[use-document-commands.test.tsx](../src/renderer/src/hooks/use-document-commands.test.tsx) and
[window-lifecycle.test.ts](../src/renderer/src/platform/window-lifecycle.test.ts).
`corrupt_temp_only_becomes_a_discardable_quarantined_record` and Unix
`failed_temp_quarantine_preserves_bytes_and_can_be_retried` in
[recovery_store_tests.rs](../src-tauri/src/recovery_store_tests.rs) exercise actual files, retained
bytes, a subsequent consumer and permission failure/retry.

The Chromium crash suite uses `Page.crash`. The `@core-interaction` recovery cases run in Chromium,
Firefox and WebKit. Native tests cover file/lock operations and failure preservation, not a physical
power cut or every filesystem's persistence semantics. The bounded-close test in
[window-lifecycle.test.ts](../src/renderer/src/platform/window-lifecycle.test.ts) uses a cleanup promise
that never resolves and verifies quit dispatch; it is not a stalled-disk native UI test. These suites
do not establish WKWebView/WebView2, private-storage or bfcache-eviction guarantees. Platform runs and
actual verification results belong in the change report, not as permanent claims here.
